...
class ColoredFormatter(logging.Formatter):
    """Formatter that outputs colorized logs for console readability."""
    COLORS = {
        "DEBUG": "\033[36m",     # Cyan
        "INFO": "\033[32m",      # Green
        "WARNING": "\033[33m",   # Yellow
...
def setup_logging(log_level: Optional[str] = None, log_file: Optional[str] = None):
    """Setup logging with console + file handlers using settings from config."""
    # Use values from settings if not passed explicitly
    if log_level is None:
        log_level = settings.log_level
    ...
    if log_file:
        ...
        # Separate error log
        error_log_file = str(log_file).replace(".log", "_errors.log")
        error_handler = logging.FileHandler(error_log_file)
        error_handler.setLevel(logging.WARNING)
